 ...
 function RanderEmployees(EmpArr) {
     ...
     EmpArr.forEach(e => {
       ...
       let statusText = e.empStatus ? 'פעיל' : 'לא פעיל'; // set text based on empStatus
         ...
         str += `<button onclick="DeleteEmployee(${e.empNum})" id="delete${e.empNum}" style="background-color: red; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Delete</button>`;
         str += `<button onclick="UpdateEmployee(${e.empNum})" id="update${e.empNum}" style="background-color: green; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Update</button>`;
  ...
  function GetByStatus(EmpArr){
    ...
    if (employee.empStatus==booleanValue) {     
      ...
      str += `<div class="Empfooter">`;
      str += `<button onclick="DeleteEmployee(${employee.empNum})" id="delete${employee.empNum}" style="background-color: red; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Delete</button>`;
      str += `<button onclick="UpdateEmployee(${employee.empNum})" id="update${employee.empNum}" style="background-color: green; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer;">Update</button>`;
      str += `<h4 style="color: ${statusColor}"><strong>${statusText}</strong></h4>`;
      str += `</div></div>`; 
    }
    });   
  ...
  function UpdateEmployee(empNum){
   
    sessionStorage.setItem('UpdateEmpNum',empNum);
    location.assign('../CLIENT/EditDetailsPageAdmin.html'); 
  }
