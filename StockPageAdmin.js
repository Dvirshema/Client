...
$(document).ready(function () {
  ...
  $("#updateForm_container").submit(function () {
    let materialNum = $("#matNum").val();
    let amount = $("#matAmount").val();
    let apiPUT = `https://proj.ruppin.ac.il/cgroup42/prod/api/Material/UpdateMatiralAdmin?numMat=${materialNum}&amount=${amount}`;
...
function UpdateTheMatirelByDate(report) {
  let apiPUT = `https://proj.ruppin.ac.il/cgroup42/prod/api/Material/UpdateMatiral?numMat=${materialNum}&amount=${amount}`;
...
function RanderMaterials(MatArr) {
  ...
  MatArr.forEach((m) => {
    ...
    str += `<div id="matDiv">`;
    str += `<h4><button onclick="UpdateMaterial(${m.materialNum})" id="updatBTN">עדכון</button><strong>${m.materialName}</strong></h4>`;
...
function Suc(suc) {
  ...
  location.assign("../CLIENT/StockPageAdmin.html");
}
...
function UpdateMaterial(matNum) {
  $("#popUp").fadeIn(1000);
  document.getElementById("matNum").value = matNum;
  document.getElementById("matNum").disabled = true;
}
